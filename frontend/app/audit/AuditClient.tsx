...
function AuditPageInner({ initialSearch }: { initialSearch: string }) {
  const router = useRouter();
  // NOT useSearchParams: that hook suspends the whole page during streaming
  // SSR, so a reader without JavaScript saw the empty Suspense fallback on a
  // record of thousands of rows. The server page passes the query string in;
  // back/forward navigation updates it via popstate; our own filter writes
  // update it directly alongside router.replace.
...
export function AuditClient({ initialSearch }: { initialSearch: string }) {
  return <AuditPageInner initialSearch={initialSearch} />;
}
