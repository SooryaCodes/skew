  ...
  return (
    <section aria-label="Built on" className="relative z-10 border-y border-[color:var(--line)]">
      <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-center px-6 py-7">
        <span className="mono mr-6 text-[9px] uppercase tracking-[0.25em] text-[color:var(--text-dim)]">
          built on
        </span>
        {STACK.map((name, i) => (
          <span key={name} className="mono flex items-center text-[11px] text-[color:var(--text-dim)]">
            {i > 0 && (
